...
namespace osapi
{
  ...
  void Thread::start()
  {
    ...
    if(getuid() == 0) // Check to see if we are root
    {
      ...
      if (pthread_create(&threadId_, &attr, ThreadFunctor::threadMapper, tf_)) throw ThreadError();
      
      pthread_attr_destroy(&attr);
    }
    else
    {
      if (pthread_create(&threadId_, NULL, ThreadFunctor::threadMapper, tf_)) throw ThreadError(); 
    }
