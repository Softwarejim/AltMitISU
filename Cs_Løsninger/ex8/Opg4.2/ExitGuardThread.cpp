...
void ExitGuardThread::handleReq(exitDoorOpenReq* req)                    //Handler for ID_EXIT_DOOR_OPEN_REQ
{
    ...
    shared_ptr<exitDoorOpenCfm> cfm (new exitDoorOpenCfm);                  //Opretter ny dynamisk message pointer
    cfm.get()->exitDoorOpen = true;                                    // sætter objekts attribut =true. Exit door er åben. 
    req->whoIsAsking->send(ID_EXIT_DOOR_OPEN_CFM, cfm.get());          //send confirm message to exitguard queue.
}
