...
void EntryGuardThread::handleReq(entryDoorOpenReq* req)                  //Handler for ID_ENTRY_DOOR_OPEN_REQ
{
    ...
    shared_ptr<entryDoorOpenCfm> cfm(new entryDoorOpenCfm);                 //Opret dynamisk ny message objekt pointer                   
    cfm.get()->entryDoorOpen = true;                                  // message objektets attribut sættes. indicate that door is open          
    req->whoIsAsking->send(ID_ENTRY_DOOR_OPEN_CFM, cfm.get());          // send confirm message to car queue
