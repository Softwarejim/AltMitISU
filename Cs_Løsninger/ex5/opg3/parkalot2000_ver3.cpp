...
void *car_function(void *d)
{
    ...
    for(;;)
    {
        ...
        pthread_cond_signal(&entry_signal);             // signal to entry guard that there is available space
...
void *EntryGuard_function(void *d)
{
    for(;;)
    {
        ...
        while(!carWaitingByEntry)                       // wait untill a car signals that it is waiting by the door
            pthread_cond_wait(&entry_signal, 
                              &entry_mutex);
        while(availableSpaces <= 0)                     // wait untill there is available space in parking lot
            pthread_cond_wait(&entry_signal, 
                              &entry_mutex);
