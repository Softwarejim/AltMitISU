...
namespace osapi
{
  ...
  {
    if (pthread_cond_signal(&cond_)) throw ConditionalError();
  }

  void Conditional::broadcast()
  {
    if (pthread_cond_broadcast(&cond_)) throw ConditionalError();
  }

  void Conditional::wait(Mutex& mut)
  {
    if (pthread_cond_wait(&cond_, &mut.mut_)) throw ConditionalError();
  }
...
Conditional::~Conditional()
  {
    if (pthread_cond_destroy(&cond_)) throw ConditionalError();
  }
